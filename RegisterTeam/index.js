...
module.exports = async function (context, req) {
    ...
    if (req.body.hasOwnProperty('name') && req.body.hasOwnProperty('instanceId')) {
        ...
        try {
            ...
            let existingTeams = await storage.queryEntities(tableSvc, 'teams', new azure.TableQuery().where('name eq ? and instanceId eq ?', teamName, instanceId));
            if (existingTeams.entries.length) {
                throw Error('A team of the same name is already registered in this instance.');
            }

            let uuid = uuidv1();
            await storage.insertEntity(tableSvc, 'teams', {
                PartitionKey: 'teams',
                RowKey: uuid,
                name: teamName,
                score: 0,
