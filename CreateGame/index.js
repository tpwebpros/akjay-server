...
module.exports = async function (context, req) {
    ...
    {  
        ...
        try {
            await storage.createTable(tableSvc, gameTableName);
            await storage.createTable(tableSvc, teamTableName);

            let existingGames = await storage.queryEntities(tableSvc, gameTableName, new azure.TableQuery().where('name eq ?', gameName));
            if (existingGames.entries.length) {
                throw Error('Game name already registered');
            }

            let uuid = uuidv1();
            await storage.insertEntity(tableSvc, gameTableName, {
                PartitionKey: 'games',
                RowKey: uuid,
                name: gameName,
                maxScore
